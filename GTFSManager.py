...
#------------------Added by Haydar Çakan 29.06.2022-----------------------------------------
# The allowed users are buried in the 'credentials_mywork.py' file in the same directory and 'credentials' dictionary is used for this purpose
# To add or delete new users, see the comments on the 'credentials_mywork.py' file
from credentials_mywork import credentials
#-------------------------------------------------------------------------------------------


# The following are the Class objects to be used in each class
# RequestHandler is used to take requests from the HTTP server and Application is used to specify the paths for API's
# tornado_http_auth is imported to send auth challenges whenever '@authrequired' decorator is called and its forked under the 'BasicAuthMixin' Class in the 'tornado-http_auth.py' python package
# tornado_http_auth package documantation can be found in 'https://github.com/gvalkov/tornado-http-auth'
#------------------Added by Haydar Çakan 29.06.2022-----------------------------------------
from tornado.web import RequestHandler, Application
from tornado_http_auth import DigestAuthMixin, auth_required

#from tornado_http_auth import BasicAuthMixin2,, auth_required2
...
#-----------------------Added by Haydar Çakan 29.06.2022----------------------------------------
#___________________________START OF MAIL SERVICE PART__________________________________________

'''
Following 3 modules are used if you want to use gmail account
'''
...
'''
The following 2 modules will be imported iff you want to use mail service
random is imported to generate 6 digit unique code and smtplib is used for sending mail protocol
'''
#import random
#import smtplib
...
#-------------------starting server for SMTP mail sending protocol-----------------------------
#---Added by Haydar Çakan 29.06.2022-----------------------------------------------------------
#server= smtplib.SMTP('smtp.gmail.com',587) #server initialization for gmail and port specification (will be diferent if you use different mail services rather than gmail)

#server.starttls() #to start server with tls protocol

#server.login('staticgtfsaselsan@gmail.com','qpgefosqhjlsuhdt') #the first argument is mail name and the second one is device specfic password
#----------------This device specific 16 digit password can be taken following the https://support.google.com/mail/answer/185833?hl=en
#----------------------------------------------------------------------------------------------


#If you want to use mail service,
# 1) uncomment the following code part
# 2) comment the already existing MySaticFileHandler Class
'''
count_mail=0

class MyStaticFileHandler(tornado.web.StaticFileHandler, BasicAuthMixin, BasicAuthMixin2):

    @auth_required(realm='Protected', auth_func=credentials.get)
    def validate_absolute_path(self, root, absolute_path):
        global count_mail
        #global six_digit_code
        global unique_credential

        if count_mail==0: #to avoid sending multiple mails
            count_mail=count_mail+1
            user_mail=self.current_user
            six_digit_code=str(random.randint(100000,999999))
            server.sendmail(
                'staticgtfsaselsan@gmail.com',
                user_mail,
                "Hello,\n\nYour unique code is \""+ six_digit_code+"\"")
            print('Mail sent with unique code: '+six_digit_code)
            unique_credential={six_digit_code:six_digit_code}
            #global credentials
            #credentials={self.current_user:six_digit_code}

        print(unique_credential)

        @auth_required2(realm2='Protected2', auth_func2=unique_credential.get)

        def Inner_Static_Route(self, root, absolute_path):
        ...
        return Inner_Static_Route(self, root, absolute_path)
        '''
#___________________________END OF MAIL SERVICE PART______________________________________
#-----------------------Added by Haydar Çakan 29.06.2022----------------------------------


'''
The MyStaticFileHandler class and the following Classes are modified to sent authentication challenge

BasicAuthMixin Class from tornado_http_auth module is added to recall '@auth_required' decorator

If you want to use another credentials, define a new dictionary like 'credentials_mywork.py/credentials' and change the argument of @auth_required
Example: you defined a new credentials dictionary at
new_credentials.py/new_credentials
first you should import it:
from new_credentials.py import new_credentils
Then you should replace the decorators
@auth_required(realm='Protected', auth_func=new_credentials.get)
'''
#-----------------------Added by Haydar Çakan 29.06.2022----------------------------------

class MyStaticFileHandler(tornado.web.StaticFileHandler, DigestAuthMixin ):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')

    #@auth_required(realm='Protected', auth_func=credentials.get) #raises an auth challenge untill user enters allowed names correctly
    #--------Added by Haydar Çakan 29.06.2022--------
    def validate_absolute_path(self, root, absolute_path):
        if absolute_path.endswith('.py') or any([ (x in absolute_path) for x in forbiddenPaths]):
            # raise tornado.web.HTTPError(403) # this is the usual thing: raise 403 Forbidden error. But instead..
...
'''
If you want to add a new Class for any purpose,
you should add 'BasicAuthMixin' Class as argument and directly at start of the class
call the decorator as following to provide authentication to the new defined Class
@auth_required(realm='Protected', auth_func=credentials.get)

For the following Classes, they were alredy in the given code, however, the only thing done to other
classes is above operation, which is adding 'BasicAuthMixin' class as object and recalling '@auth_required' decorator
'''
#-----------------------Added by Haydar Çakan 29.06.2022----------------------------------
...
class allStops(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class allStopsKeyed(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class routes(tornado.web.RequestHandler,DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    def get(self):
        start = time.time()
...
class fareAttributes(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class fareRules(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class fareRulesPivoted(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class agency(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class sequence(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class trips(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class stopTimes(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class routeIdList(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class tripIdList(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class calendar(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class serviceIds(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class stats(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class gtfsImportZip(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def post(self):
...
class commitExport(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class pastCommits(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
...
class XMLUpload(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def post(self):
...
class XMLDiagnose(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class stations(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class fareChartUpload(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def post(self):
...
class xml2GTFS(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def post(self):
...
class gtfsBlankSlate(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class translations(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class shapesList(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class shape(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def post(self):
...
class allShapesList(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class listAll(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class zoneIdList(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class diagnoseID(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class deleteByKey(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class replaceID(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class hydGTFS(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def post(self):
...
class frequencies(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class tableReadSave(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
...
class tableColumn(tornado.web.RequestHandler, DigestAuthMixin):
    def prepare(self):
        self.get_authenticated_user(check_credentials_func=credentials.get, realm='Protected')
    #@auth_required(realm='Protected', auth_func=credentials.get)
    def get(self):
